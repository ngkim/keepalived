...
/* Global vars */
bfd_data_t *bfd_data = NULL;
bfd_data_t *old_bfd_data = NULL;
char *bfd_buffer = NULL;

static bfd_t * find_bfd_by_name2(char *, bfd_data_t *);
...
/* Initialize bfd_t */
void
alloc_bfd(char *name, int disabled)
{
	bfd_t *bfd = NULL;
	assert(name);
	int size = strlen(name);
	bfd = (bfd_t *) MALLOC(sizeof (bfd_t));
	memcpy(bfd->iname, name, size);
...
bfd_complete_init(void)
{
	...
	for (e = LIST_HEAD(bfd_data->bfd); e; ELEMENT_NEXT(e)) {
		...
		if (reload)
			bfd_old = find_bfd_by_name2(bfd->iname, old_bfd_data);
		if (reload && bfd_old) {
			bfd_copy_state(bfd_old, bfd);
			bfd_copy_sands(bfd_old, bfd);
...
bfd_t *
find_bfd_by_discr(u_int32_t discr)
{
	element e = NULL;
	bfd_t *bfd = NULL;
...
u_int32_t
rand_intv(u_int32_t min, u_int32_t max)
{
